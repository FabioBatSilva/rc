...
%% states
-export([prepare/3, execute/2, waiting/2]).

-record(state, {vnode_master,
                cmd,
                from,
                preflist,
                num,
...
do(VnodeMaster, Cmd, N, WR) ->
    {ok, Pid} = rc_sup:start_coordinator(VnodeMaster),
...
prepare({Cmd, N, WR}, From, State) ->
    ...
    {next_state, execute, State#state{cmd=Cmd,
                                      from=From,
                                      preflist=Preflist,
                                      num=WR}, 0}.

execute(timeout, State) when State#state.num == 0 -> % async
    riak_core_vnode_master:command(State#state.preflist,
                                   State#state.cmd,
...
waiting(Res, State) ->
    ...
    if length(Replies) /= State#state.num -> {next_state, waiting, State#state{replies=Replies}};
       true ->
